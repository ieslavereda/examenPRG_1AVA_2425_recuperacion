    ...
    public static boolean ejercicio1(String password) {
        ...
        for (int i = 0; i < password.length(); i++) {
            for (int j = 0; j < letrasMinusculas.length(); j++) {
                if (!minusculas && password.charAt(i) == letrasMinusculas.charAt(j)) {
                    minusculas = true;
                }
            }
            for (int k = 0; k < LETRAS_MAYUSCULAS.length(); k++) {
                if (!mayusculas && password.charAt(i) == LETRAS_MAYUSCULAS.charAt(k)) {
                    mayusculas = true;
                }
            }
            for (int l = 0; l < NUMEROS.length(); l++) {
                if (!numeros && password.charAt(i) == NUMEROS.charAt(l)) {
                    numeros = true;
                }
            }
        }
